...
# File:     clean_diary12.R
# About:    Script to clean travel diary 1 & 2 survey
########################################
...
temp <- read.csv(file=file.path("Data", "Survey 2019 Winter", "Data 0 Raw", 
                                "2019 Winter Transportation Study -- Travel Diary Survey_May 23, 2019_15.52.csv"), 
                 stringsAsFactors=F)
temp <- temp[-c(1:2),]
# names
...
temp$IDLE_MIN[temp$IDLE_MIN=="5 minutes"] <- 5
...
table(temp$HHPER %in% PER$HHPER) # 21
...
# Change names
diary12 <- temp
diary12_names <- temp_names
row.names(diary12) <- NULL
...
saveRDS(diary12, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary12.rds"))
write.csv(diary12, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary12.csv"), row.names=F)
write.csv(diary12_names, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary12_names.csv"), row.names=F)
rm(t_folder)
