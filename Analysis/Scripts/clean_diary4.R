...
# File:     clean_diary4.R
# About:    Script to clean travel diary 4 survey
########################################
...
temp <- read.csv(file=file.path("Data", "Survey 2019 Winter", "Data 0 Raw", 
                                "2019 Winter Transportation Study -- Day 4 Travel Diary Survey_May 23, 2019_15.53.csv"), 
                 stringsAsFactors=F)
temp <- temp[-c(1:2),]
# names
...
temp$IDLE_MIN[temp$IDLE_MIN=="3 min"] <- 3
temp$IDLE_MIN[temp$IDLE_MIN=="5 min "] <- 5
...
table(temp$HHPER %in% PER$HHPER) # 4
...
# Change names
diary4 <- temp
diary4_names <- temp_names
row.names(diary4) <- NULL
...
saveRDS(diary4, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary4.rds"))
write.csv(diary4, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary4.csv"), row.names=F)
write.csv(diary4_names, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary4_names.csv"), row.names=F)
rm(t_folder)
