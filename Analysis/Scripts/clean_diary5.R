...
# File:     clean_diary5.R
# About:    Script to clean travel diary 5 survey
########################################
...
temp <- read.csv(file=file.path("Data", "Survey 2019 Winter", "Data 0 Raw", 
                                "2019 Winter Transportation Study -- Day 5 Travel Diary Survey_May 23, 2019_15.53.csv"), 
                 stringsAsFactors=F)
temp <- temp[-c(1:2),]
# names
...
table(temp$HHPER %in% PER$HHPER) # 7
...
# Change names
diary5 <- temp
diary5_names <- temp_names
row.names(diary5) <- NULL
...
saveRDS(diary5, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary5.rds"))
write.csv(diary5, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary5.csv"), row.names=F)
write.csv(diary5_names, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary5_names.csv"), row.names=F)
rm(t_folder)
