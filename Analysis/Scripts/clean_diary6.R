...
# File:     clean_diary6.R
# About:    Script to clean travel diary 6 survey
########################################
...
temp <- read.csv(file=file.path("Data", "Survey 2019 Winter", "Data 0 Raw", 
                                "2019 Winter Transportation Study -- Day 6 Travel Diary Survey_May 23, 2019_15.54.csv"), 
                 stringsAsFactors=F)
temp <- temp[-c(1:2),]
# names
...
table(temp$HHPER %in% PER$HHPER) # 3
...
# Change names
diary6 <- temp
diary6_names <- temp_names
row.names(diary6) <- NULL
...
saveRDS(diary6, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary6.rds"))
write.csv(diary6, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary6.csv"), row.names=F)
write.csv(diary6_names, file=file.path("Data", "Survey 2019 Winter", t_folder, "diary6_names.csv"), row.names=F)
rm(t_folder)
